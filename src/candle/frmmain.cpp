...
//    ui->scrollArea->updateMinimumWidth();
    ...
    ui->cmdAMinus->setBackColor(QColor(210, 186, 153));
    ui->cmdAPlus->setBackColor(ui->cmdAMinus->backColor());
...
{
    if (MultiStatus == false) ui->txtCountRun->setValue(0);
    ...
    while (m_serialPort.canReadLine()) {
        ...
        if (data[0] == '<') {
            ...
            if (mpx.indexIn(data) != -1) {
                ...
                ui->txtMPosA->setValue(mpx.cap(4).toDouble());
            ...
            ui->txtWPosA->setValue(ui->txtMPosA->value() - aPos);
...
void frmMain::loadSettings()
{
    ...
    int c = b * 0.8;
    setStyleSheet(styleSheet() + QString(R"CSS(
...
void frmMain::on_cmdFileMultiSend_clicked()
{
    MultiStatus = true;
    if (ui->txtCountRun->value() <= 0) MultiStatus = false;
    if (ui->txtCountRun->value() > 0) on_cmdFileSend_clicked();
}
...
void frmMain::jogStep()
{
    if (ui->cboJogStep->currentText().toDouble() != 0) {
        ...
        double r_vec = r_jogVector * ui->cboJogStep->currentText().toDouble();

        if (vec.length()||r_vec) {
            sendCommand(QString("$J=%6G91X%1Y%2Z%3A%4F%5")
...
void frmMain::completeTransfer()
{
    ...
    ui->txtCountRun->setValue(ui->txtCountRun->value() - 1);
    if (ui->txtCountRun->value() > 0) on_cmdFileSend_clicked();
...
QString frmMain::getLineInitCommands(int row)
{
    ...
    static QRegExp getStartA("A\\d*(.\\d*)");
    static QRegExp getMValue("M\\d");
    QString StartA = "";
    QString MValue = "";
    QVariant ActiveGcode = "";
    ...
    if (lineNumber != -1) {
        ...
        segmentIndex = row;
        while (segmentIndex > 2 && StartA == "") // find previos A axis
        {
            --segmentIndex;
            QModelIndex rowIdx= ui->tblProgram->model()->index(segmentIndex,1);
            ActiveGcode = ui->tblProgram->model()->data(rowIdx);
            if ((getStartA.indexIn(ActiveGcode.toString()) != -1) && StartA == "") StartA = QString(getStartA.cap(0)) ;
        }
        segmentIndex = row;
        while (segmentIndex > 0 && MValue == "") // find start motor command
        {
            --segmentIndex;
            QModelIndex rowIdx= ui->tblProgram->model()->index(segmentIndex,1);
            ActiveGcode = ui->tblProgram->model()->data(rowIdx);
            if ((getMValue.indexIn(ActiveGcode.toString()) != -1) && MValue == "") MValue = QString(getMValue.cap(0)) ;
        }
        if (MValue == "") MValue = "M3";

        commands.append(QString(MValue + " S%1\n").arg(qMax<double>(lastSegment->getSpindleSpeed(), ui->slbSpindle->value())));

        commands.append(QString("G21 G90 G0 X%1 Y%2 ")
                        .arg(firstSegment->getStart().x())
                        .arg(firstSegment->getStart().y()));

        commands.append(QString(StartA + "\n"));

        commands.append(QString("Z%1\n")
