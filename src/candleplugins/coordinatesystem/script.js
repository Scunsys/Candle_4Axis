...
function onAppSettingsLoaded()
{
    ...
    uiPanel.txtOffsetA.decimals = u ? 4 : 3;
    ...
    uiPanel.txtOffsetA.mimimum = -b;
    uiPanel.txtOffsetA.maximum = b;
}
...
function onAppResponseReceived(command, index, response)
{
    function displayOffsets(r) {
        var gx = new RegExp(currentCS + ":([\\d\\.\\-]+),([\\d\\.\\-]+),([\\d\\.\\-]+),([\\d\\.\\-]+)");
        var s = r.match(gx);
        if (s) {
            for (var i = 0; i < s.length; i++) {
                ...
                uiPanel.txtOffsetA.value = parseFloat(s[4]);
            }
        }
    }
